...
class CalendarBuilder {
    ...
    private buildCalendar(): Array<Array<Day>> {
        const currentDate = new Date(this.config.startingYear, 0, 1);
        const calendar: Array<Array<Day>> = [];
    ...
    private addNotStaticHolidays(calendar: Array<Array<Day>>) {
        for (let i = 0; i < calendar.length; i++) {
            for (let j = 0; j < calendar[i].length; j++) {
                const date: Date = calendar[i][j].getDate();

                for (let k = 0; k < notStaticholidays.length; k++) {
                    if (
                        date.getMonth() == notStaticholidays[k].month &&
                        date.getDate() == notStaticholidays[k].day
                    ) {
                        switch (date.getDay()) {
                            case 2:
                                calendar[i][j - 1] = new Holiday(
                                    calendar[i][j - 1],
                                    notStaticholidays[k].description
                                );
                                break;
                            case 3:
                                calendar[i][j - 2] = new Holiday(
                                    calendar[i][j - 2],
                                    notStaticholidays[k].description
                                );
                                break;
                            case 4:
                                calendar[i][j + 4] = new Holiday(
                                    calendar[i][j + 4],
                                    notStaticholidays[k].description
                                );
                                break;
                            case 5:
                                calendar[i][j + 3] = new Holiday(
                                    calendar[i][j + 3],
                                    notStaticholidays[k].description
                                );
                                break;
                            default:
                                calendar[i][j] = new Holiday(
                                    calendar[i][j],
                                    notStaticholidays[k].description
                                );
                                break;
